...
namespace VRTK
{
    ...
    public class VRTK_MoveInPlace : MonoBehaviour
    {
        ...
        public virtual void SetControlOptions(ControlOptions givenControlOptions)
        {
            ...
            {
                VRTK_SharedMethods.AddListValue(ref trackedObjects, VRTK_DeviceFinder.GetActualController(controllerLeftHand).transform, true);
                VRTK_SharedMethods.AddListValue(ref trackedObjects, VRTK_DeviceFinder.GetActualController(controllerRightHand).transform, true);
            }

            if (headset != null && (controlOptions == ControlOptions.HeadsetAndControllers || controlOptions == ControlOptions.HeadsetOnly))
            {
                VRTK_SharedMethods.AddListValue(ref trackedObjects, headset.transform, true);
            }
        }
        ...
        protected virtual void OnEnable()
        {
            ...
            for (int i = 0; i < trackedObjects.Count; i++)
            {
                ...
                VRTK_SharedMethods.AddDictionaryValue(ref movementList, trackedObj, new List<float>(), true);
                VRTK_SharedMethods.AddDictionaryValue(ref previousYPositions, trackedObj, trackedObj.transform.localPosition.y, true);
            }
            if (playArea == null)
            {
        ...
        protected virtual float CalculateListAverage()
        {
            ...
            for (int i = 0; i < trackedObjects.Count; i++)
            {
                ...
                // Convenience code.
                List<float> trackedObjList = VRTK_SharedMethods.GetDictionaryValue(movementList, trackedObj, true, new List<float>());

                // Cap off the speed.
                if (deltaYPostion > sensitivity)
                {
                    VRTK_SharedMethods.AddListValue(ref trackedObjList, sensitivity, true);
                }
                else
                {
                    VRTK_SharedMethods.AddListValue(ref trackedObjList, deltaYPostion, true);
                }
        ...
        protected virtual void EngageButtonReleased(object sender, ControllerInteractionEventArgs e)
        {
            ...
            {
                Transform trackedObj = trackedObjects[i];
                VRTK_SharedMethods.GetDictionaryValue(movementList, trackedObj, true, new List<float>()).Clear();
            }
            initialGaze = Vector3.zero;
