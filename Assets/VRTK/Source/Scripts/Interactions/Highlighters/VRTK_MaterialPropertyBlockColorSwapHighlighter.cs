    ...
    public class VRTK_MaterialPropertyBlockColorSwapHighlighter : VRTK_MaterialColorSwapHighlighter
    {
        ...
        public override void Unhighlight(Color? color = null, float duration = 0f)
        {
            ...
            for (int i = 0; i < renderers.Length; i++)
            {
                ...
                MaterialPropertyBlock storedPropertyBlock = VRTK_SharedMethods.GetDictionaryValue(originalMaterialPropertyBlocks, objectReference, false, null);
        ...
        protected override void StoreOriginalMaterials()
        {
            ...
            for (int i = 0; i < renderers.Length; i++)
            {
                ...
                VRTK_SharedMethods.AddDictionaryValue(ref originalMaterialPropertyBlocks, objectReference, blankPropertyBlock, true);
                VRTK_SharedMethods.AddDictionaryValue(ref highlightMaterialPropertyBlocks, objectReference, blankPropertyBlock, true);
        ...
        protected override void ChangeToHighlightColor(Color color, float duration = 0f)
        {
            ...
            for (int i = 0; i < renderers.Length; i++)
            {
                ...
                if (VRTK_SharedMethods.GetDictionaryValue(originalMaterialPropertyBlocks, faderRoutineID, false, null) == null)
                {
                    continue;
                }

                Coroutine existingFaderRoutine = VRTK_SharedMethods.GetDictionaryValue(faderRoutines, faderRoutineID, false, null);
                if (existingFaderRoutine != null)
                {
                    StopCoroutine(existingFaderRoutine);
                    VRTK_SharedMethods.RemoveDictionaryKey(ref faderRoutines, faderRoutineID);
                }
                ...
                if (duration > 0f)
                {
                    VRTK_SharedMethods.AddDictionaryValue(ref faderRoutines, faderRoutineID, StartCoroutine(CycleColor(renderer, highlightMaterialPropertyBlock, color, duration)), true);
                }
                else
                {
