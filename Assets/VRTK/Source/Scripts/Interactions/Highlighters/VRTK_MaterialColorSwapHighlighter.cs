    ...
    public class VRTK_MaterialColorSwapHighlighter : VRTK_BaseHighlighter
    {
        ...
        protected virtual void StoreOriginalMaterials()
        {
            ...
            for (int i = 0; i < renderers.Length; i++)
            {
                Renderer renderer = renderers[i];
                string objectReference = renderer.gameObject.GetInstanceID().ToString();
                VRTK_SharedMethods.AddDictionaryValue(ref originalSharedRendererMaterials, objectReference, renderer.sharedMaterials, true);
                VRTK_SharedMethods.AddDictionaryValue(ref originalRendererMaterials, objectReference, renderer.materials, true);
            ...
            for (int j = 0; j < renderers.Length; j++)
            {
                ...
                for (int i = 0; i < renderer.materials.Length; i++)
                {
                    ...
                    if (material.HasProperty("_Color"))
                    {
                        if (duration > 0f)
                        {
                            VRTK_SharedMethods.AddDictionaryValue(ref faderRoutines, faderRoutineID, StartCoroutine(CycleColor(material, material.color, color, duration)), true);
                        }
                        else
                        {
