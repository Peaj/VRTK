...
#endif
    {
        ...
        protected virtual SteamVR_TrackedObject GetTrackedObject(GameObject controller)
        {
            ...
            else
            {
                ...
                if (trackedObject != null)
                {
                    VRTK_SharedMethods.AddDictionaryValue(ref cachedTrackedObjectsByGameObject, controller, trackedObject, true);
                    VRTK_SharedMethods.AddDictionaryValue(ref cachedTrackedObjectsByIndex, (uint)trackedObject.index, trackedObject, true);
                }
        ...
        protected virtual bool CheckAxisTouch(uint index, ButtonPressTypes type, EVRButtonId axisId)
        {
            ...
            bool currentAxisPressState = VRTK_SharedMethods.GetDictionaryValue(axisTouchStates, axisId, true, false);
            float axisFidelity = VRTK_SharedMethods.GetDictionaryValue(axisTouchFidelity, axisId, false, 0f);
            ...
            switch (type)
            {
                ...
                case ButtonPressTypes.TouchDown:
                    ...
                    {
                        VRTK_SharedMethods.AddDictionaryValue(ref axisTouchStates, axisId, true);
                        return true;
                    }
                    return false;
                case ButtonPressTypes.TouchUp:
                    if (currentAxisPressState && VRTK_SharedMethods.Vector3ShallowCompare(axisValue, Vector2.zero, axisFidelity))
                    {
                        VRTK_SharedMethods.AddDictionaryValue(ref axisTouchStates, axisId, false);
                        return true;
                    }
                    return false;
            }
